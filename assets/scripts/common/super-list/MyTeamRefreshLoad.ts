...
@ccclass('MyTeamRefreshLoad')
export class MyTeamRefreshLoad extends BaseMain {
    ...
    onEnable () {
        ...
        Message.on("SeachTeamDataSucc",this.seachTeamDataSucc,this)
    }
    ...
    onDisable () {
        ...
        Message.off("SeachTeamDataSucc",this.seachTeamDataSucc,this)
    }

    seachTeamDataSucc (event:string,args:any) {
        this.page = 1
        this.datas = []
        for (let i=0; i<args.player_list.length; i++) {
            this.datas.push(args.player_list[i])
        }
        this.scheduleOnce(() => this.layout.total(this.datas.length), 0.2)
    }
    ...
    requestData (callback?:any) {
        httpRequest.post("api/v1/promotion-team",
            ...
            (succ:any) => {
                if (0 < succ.player_list.length) {
                    this.page += 1
                }
                for (let i=0; i<succ.player_list.length; i++) {
                    succ.player_list[i].maxRatio = Number(succ.max_ratio)
                    this.datas.push(succ.player_list[i])
                }
