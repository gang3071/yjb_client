...
export class SuperLayout extends Component {
    ...
    get transform(): UITransform | null { return this.node.getComponent(UITransform) }
    /** View 可容纳的宽度 */
    get accommodWidth() {
    ...
    get header(): UITransform | null {
        ...
        return this.node.children[0].getComponent(UITransform)
    }
    /** 底部的节点 */
    get footer(): UITransform | null {
        if (this.node.children.length == 0) return null
        return this.node.children[this.node.children.length - 1].getComponent(UITransform)
    }
    /** 头部索引 */
    ...
    get centerPosition(): Vec3 {
        if (!this._centerPosition) {
            ...
            if (this.autoCenter) {
                if (this.centerNode) {
                    let worldPos = this.centerNode.parent?.getComponent(UITransform)?.convertToWorldSpaceAR(this.centerNode.position)!
    ...
    updateItemSize(node: Node, size: Size) {
        ...
        this.onChangeChildSize(node.getComponent(UITransform))
    }
    /** 自动居中到最近Item */
    scrollToCenter() {
        this.soonFinish()
    ...
    protected createItems(count: number, refreshLastItem: boolean = false) {
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (child["__index"] == 0) {
                ...
                relative = this.node.children[index].getComponent(UITransform)
            }
        ...
        if (!isNaN(this.stretchLock.index!)) {
            ...
            if (index != -1) {
                for (let i = index; i >= 0; i--) {
                    ...
                    if (i == index) continue
                    if (i < index) {
                        this.setItemPosition(item.getComponent(UITransform), this.node.children[i + 1].getComponent(UITransform), true)
                    }
                }
                for (let i = index; i < this.node.children.length; i++) {
                    const item = this.node.children[i];
                    if (i == index) continue
                    this.setItemPosition(item.getComponent(UITransform), this.node.children[i - 1].getComponent(UITransform))
                }
                return
            }
        }
