...
@ccclass('PromotionDataRefreshLoad')
export class PromotionDataRefreshLoad extends BaseMain {
    ...
    requestData (callback?:any) {
        httpRequest.post("api/v1/promotion-data",
            {page:this.page,size:this.size},
            (succ:any) => {
                if (0 < succ.settlement_record.length) {
                    this.page += 1
                }
                for (let i=0; i<succ.settlement_record.length; i++) {
                    this.datas.push(succ.settlement_record[i])
                }
                if (0 < this.datas.length) {
                    this.scheduleOnce(() => this.layout.total(this.datas.length), 0.2)
                }
                if (0 == this.datas.length && 0 == succ.settlement_record.length) {
