...
import { Message, MessageEventData } from '../../base/core/MessageMgr';
...
@ccclass('SLGame')
export class SLGame extends BaseView {
    ...
    start() {
        ...
        for (let i=0; i<this.moveNode.children.length; i++) {
            ...
            node.getChildByName("desc").getComponent(Label).string = ""
        }
        this.moveNode.setPosition(v3(-this.moveNode.parent.getComponent(cc.UITransform).width/2,0,0))
        ...
        if (!this.m_uidata.machineInfo.has_spectator) {
            this.sendActionReq("pressure_score")
        }
        ...
        // this.videoSprite.setPosition(cc.v3(this.videoSprite.getPosition().x,nodeContent.height/2))
        // this.originalPos = this.videoSprite.getPosition()
        this.btnStart.on(cc.Node.EventType.TOUCH_START,(target:cc.EventTouch) => {
            let node = target.getCurrentTarget()
            node.getChildByName("img1").active = false
            node.getChildByName("img2").active = true
        })
        this.btnStart.on(cc.Node.EventType.TOUCH_CANCEL,(target:cc.EventTouch) => {
            let node = target.getCurrentTarget()
            node.getChildByName("img1").active = true
            node.getChildByName("img2").active = false
        })
        this.btnStart.on(cc.Node.EventType.TOUCH_END,(target:cc.EventTouch) => {
            let node = target.getCurrentTarget()
            node.getChildByName("img1").active = true
            node.getChildByName("img2").active = false
            this.btnStartCall()
    ...
    onDisable () {
        ...
        UIMgr.getInstance().closeView(UIConfig.SLXiaFen.path)
    ...
    eventChangeLine (event:string,args:any) {
        ...
        //this.playTcplayerVideo(succ.tencent_machine_media[args])
    ...
    UpdateData(event:string, args:any){
        if (args.id == this.m_uidata.machineInfo.id) {
            ...
            this.m_uidata.machineInfo.bet = args.machine_info.bet
    ...
    initData () {
        ...
        this.labMachineLv.string = `${succ.odds_x}:${succ.odds_y}`
        ...
        this.btnAuto.active = succ.auto_up_turn == 0?true:false
        this.btnStopAuto.active = succ.auto_up_turn == 1?true:false
    ...
    initSaveMachineNodeState (data:any) {
        this.saveMachineNode.getChildByName("normal").active = Number(data.keeping) == 0 ? true:false
        this.saveMachineNode.getChildByName("hold").active = Number(data.keeping) == 1 ? true:false
        ...
        if(minutes < 10 && 0 != Number(minutes)){
            minutes = "0" + minutes
        }
    ...
    btnMoreMachineCall () {
        ...
        UIMgr.getInstance().openSingleView(UIConfig.MachineMore.path,{machine_id:this.m_uidata.machine_id,data:this.m_uidata.machineInfo},null,null,() => {
            console.log("=sl==btnMoreMachineCall===")
            utils.backExit(this.m_uidata.machineInfo.id)
            this.close()
    ...
    sendActionReq (action:string) {
        httpRequest.post("api/v1/slot-action",{
            ...
            if ("start" == action && 1 == this.m_uidata.machineInfo.auto_up_turn) {
                AlterTipsWrap.show("开始游戏")
            }
            if ("stop_1" == action && 1 == this.m_uidata.machineInfo.auto_up_turn) {
                AlterTipsWrap.show("第一列停止")
            }
            if ("stop_2" == action && 1 == this.m_uidata.machineInfo.auto_up_turn) {
                AlterTipsWrap.show("第二列停止")
            }
            if ("stop_3" == action && 1 == this.m_uidata.machineInfo.auto_up_turn) {
                AlterTipsWrap.show("第三列停止")
            }
            if ("auto" == action) {
                this.m_uidata.machineInfo.auto_up_turn = 1
                AlterTipsWrap.show("开始自动")
            }
            if ("stop_auto" == action) {
                this.m_uidata.machineInfo.auto_up_turn = 0
        ...
        httpRequest.post("api/v1/show_open_point_rule",{
            ...
            if (succ.machineCategory.if_se_give_point) {
                if (succ.machineCategory.is_allow_client_give_point) {
                    LoadingViewWrap.show()
                    UIMgr.getInstance().openSingleView(UIConfig.ShangFen.path,{data:this.m_uidata.machineInfo,machine_id:this.m_uidata.machine_id,isGzOrSl:1,succ:succ.machineCategory})
                }else {
                    AlterTipsWrap.show("当前享受开分赠点福利，分数未用完或未达到开分总数的倍条件之前，无法再次开分")
                    return
                }
            }else {
                LoadingViewWrap.show()
    ...
    btnXiaFenCall () {
        ...
        LoadingViewWrap.show()
        UIMgr.getInstance().openSingleView(UIConfig.SLXiaFen.path,{data:this.machineData,machine_id:this.m_uidata.machine_id,isGzOrSl:1})
    }
    ...
    btnKanBiaoCall () {
        this.sendActionReq("out_1_pulse")
    }

    btnLvLiCall () {
        LoadingViewWrap.show()
        UIMgr.getInstance().openSingleView(UIConfig.GameLL.path,{data:this.machineData,machine_id:this.m_uidata.machine_id,isGzOrSl:1})
    }
