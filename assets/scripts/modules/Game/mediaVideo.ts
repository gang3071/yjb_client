...
export class MediaVideo extends Component {
    ...
    @property(Sprite)
    public render: Sprite = null;

    @property(Sprite)
    public new_render: Sprite = null;
    ...
    start() {
        ...
        this._canvas.width = this.new_render.getComponent(UITransform).width;
        this._canvas.height = this.new_render.getComponent(UITransform).height;
    ...
    resetMachie () {
        console.log("==resetMachie==",this.machineid)
        this._initialize();
    }

    setSourcesUrl (ip:string,streamId:string,machineId:number,videoIndex:number=0) {
        Message.off("ResetMachine_" + machineId,this.resetMachie,this)
        Message.on("ResetMachine_" + machineId,this.resetMachie,this)
        // console.log("==setSourcesUrl===",url)
        // this.source = url
        this.streamIp = ip
    ...
    private _initialize() {
        if (JSB) {
            ...
            utils.createVideo(this.machineid)
     ...
     */
     private _initializeBrowser(): void {
            ...
            // div.id = "kuokuo_test_video"
            // div.style.width = "100%";
            // div.style.height = "100%";
            // // document.getElementById("Cocos3dGameContainer").appendChild(div)
            // document.getElementById("GameCanvas").appendChild(div)
            // // document.body.appendChild(div)
    
            // const testScript = document.createElement("script");
            // testScript.id = "kuokuo_video_script"
            // testScript.textContent = 
            // `window.kuokuoPlayer = new TcPlayer('kuokuo_test_video', {
    
            //         m3u8: '${this.source}',
            //         // webrtc: 'webrtc://5664.liveplay.myqcloud.com/live/5664_harchar1?txSecret=f22a813b284137ed10d3259a7b5c224b&txTime=6403f7bb',
            //         autoplay: true,
            ...
            // document.getElementById("Cocos3dGameContainer").appendChild(testScript);
        ...
        for (let i=0; i<videoElementArr.length; i++) {
            if (("video_" + this.machineid) == videoElementArr[i].id) {
                this._video = videoElementArr[i]
            }
        }

        console.log("=====", this._video)
        // this._video = videoElementArr[this.videoIndex]
        ...
        let onCanPlay = () => {
            ...
            Message.dispatchEvent("PlayVideo",{machineid:this.machineid})
        };
        ...
        this._video.addEventListener('suspend', onSuspend);
        ...
        utils.createScrit(this.streamIp,this.streamId,this.machineid)
    }
    ...
    private _updateVideoSource() {
        ...
        console.log('22222_updateVideoSource', url);
        if (JSB) {
            this._video.stop();
    ...
    public onDisable(): void {
        ...
        if (document.getElementById("kuokuo_test_video")) {
            document.getElementById("kuokuo_test_video").remove()
        }
        if (document.getElementById("kuokuo_video_script")) {
            document.getElementById("kuokuo_video_script").remove()
        }
        Message.off("ResetMachine_"+this.machineid,this.resetMachie,this)
    }
    ...
    private async updateTexture() {
        ...
        try {
            ...
            this.new_render.spriteFrame = spriteFrame;
