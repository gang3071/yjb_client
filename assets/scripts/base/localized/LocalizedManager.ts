...
export const LANGUAGE_DEFAULT = LANGUAGE_TYPE.FT;
...
export class LocalizedConfigPathData extends LocalizedConfigData{
    ...
    constructor( name : string, { chiness, english, fanti, japanese} : { chiness : string, english : string,fanti : string, japanese : string}){
        super(name);
        this.m_chiness = chiness;
        this.m_english = english;
        this.m_fanti = fanti;
        this.m_japanese = japanese;
    ...
    public loadData( loader : ResLoader, language : LANGUAGE_TYPE,doneCallback : ( labelMap : any, spriteMap : any)=>void){
        loader.LoadJsonAsset(this.getFile(language), (error : Error | null, data : cc.JsonAsset)=>{
                if(error || data == null){
                    console.error("load assets failed", error?.message);
...
export class LocalizedConfigJsonData extends LocalizedConfigData{
    ...
    private m_fanti : any = null!;
    private m_japanese : any = null!;

    constructor( name : string, { chiness, english, fanti, japanese  } : { chiness : any, english : any, fanti:any, japanese:any }){
        super(name);
        this.m_chiness = chiness;
        this.m_english = english;
        this.m_fanti = fanti;
...
export class LocalizadManager extends EventDispatcher{
    ...
    public switchLanguage( language : LANGUAGE_TYPE){
        ...
        StoreMgr.getInstance().setIntValue("CURR_LANGUAGE",language)
        this.dispatch(LANGUAGE_EVENT.UPDATE);
        // });
    ...
    public getSprite(key : string) : SpriteFrameConfig | null{
        console.log("==getSprite==",this.m_spriteMap)
