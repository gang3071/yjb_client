...
import { LocalizadManager, LANGUAGE_EVENT, LANGUAGE_TYPE, LANGUAGE_DEFAULT } from './LocalizedManager';
...
export class LocalizadSprite extends cc.Component {
    ...
    protected m_ftSprite : cc.SpriteFrame = null!;
    ...
    protected m_jpSprite : cc.SpriteFrame = null!;
    ...
    protected getSpriteByLanguage( language : LANGUAGE_TYPE) : cc.SpriteFrame{
        ...
        switch(language){
            ...
            case LANGUAGE_TYPE.FT:
                {
                    spriteFrame = this.m_ftSprite;
                    break;
                }
            case LANGUAGE_TYPE.JP:
                {
                    spriteFrame = this.m_jpSprite;
                    break;
                }
        }
    ...
    protected updateFrameByConfig(){
        // console.log("======updateFrameByConfig=======",SpriteConfig[this.m_frameName])
        let index = StoreMgr.getInstance().getIntValue("CURR_LANGUAGE",LANGUAGE_DEFAULT)
        // if (index == LocalizadManager.getInstance().getLanauge()) {
        //     return
        // }

        if (SpriteConfig[this.m_frameName]) {
            let path = SpriteConfig[this.m_frameName][LocalizadManager.getInstance().getLanauge()-1]

            this.m_resLoad.pushStackAssets();

            this.m_resLoad.loadSpriteFrame(path, (error : Error | null, sprite : cc.SpriteFrame)=>{
                if(sprite){
        ...
        // console.log("==updateFrameByConfig===",spriteConfig)
